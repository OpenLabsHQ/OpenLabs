...
def test_creds_factory_non_existent_provider_type() -> None:
    ...
    aws_secrets = AWSSecrets.model_validate(bad_provider_creds_payload)
    aws_secrets.provider = "Fake Provider"  # type: ignore

    with pytest.raises(ValueError):
        _ = CredsFactory.create_creds_verification(
            credentials=aws_secrets,
        )
...
def test_creds_factory_build_aws_creds() -> None:
    ...
    aws_secrets = AWSSecrets.model_validate(aws_secrets_payload)
    creds_object = CredsFactory.create_creds_verification(credentials=aws_secrets)
