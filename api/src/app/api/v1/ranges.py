...
from ...schemas.vpn_schemas import VPNClientCreateRequest, VPNClientSchema
...
async def create_vpn_client_endpoint(
    ...
    if not deployed_range.vpn:
        logger.info(
            "Failed to create a VPN client for user: %s (%s). VPN access is not enabled for range: %s!",
            current_user.email,
            current_user.id,
            range_id,
        )
        raise HTTPException(
            ...
            detail="VPN access must be cofigured before creating VPN client configurations!",
        )

    # Queue VPN client job
    job_name = "generate_wireguard_vpn_client"
...
async def get_vpn_clients_for_range_endpoint(
    range_id: int,
    db: AsyncSession = Depends(async_get_db),  # noqa: B008
    current_user: UserModel = Depends(get_current_user),  # noqa: B008
...
async def get_specific_vpn_client_endpoint(
    range_id: int,
    client_id: int,
    db: AsyncSession = Depends(async_get_db),  # noqa: B008
    current_user: UserModel = Depends(get_current_user),  # noqa: B008
